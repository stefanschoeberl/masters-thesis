...
Objektmethoden, Klassenmethoden und Initialisierer werden auf in WebAssembly definierte Funktionen abgebildet. Grundsätzlich wird für alle drei Arten ein Typ (\lstinline{(type ...)}) im WebAssembly-Modul definiert, der die Schnittstelle der Methode bzw. des Initialisierer beschreibt. Haben mehrere Funktionen diesselbe Schnittstelle, werden Typen wiederverwendet. Jede WebAssembly-Funktion referenziert auf den entsprechenden Typ. Für diese drei Arten wird sehr ähnlicher, aber leicht unterschiedlicher Funktionsbytecode erzeugt.

Objektmethoden und Initialisierer benötigen eine Referenz auf das Objekt selbst. Diese Referenz ist im Methoden- bzw. Initialisiererrumpf über das Schlüsselwort \lstinline{this} abrufbar. Daher wird für Objektmethoden und Initialisierer ein zusätzlichen Parameter definiert, auch \lstinline{this}-Parameter genannt, über den sie die Referenz auf das Objekt zur Laufzeit erhalten können. Der \lstinline{this}-Parameter ist bei Objektmethoden und Initialisierern immer der erste Parameter.
